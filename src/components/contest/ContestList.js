...
export default function ContestList(props) {
    ...
    const { data, isLoading } = useMoralisQuery(
        "JoinContest", query => query.matches("user", user.id)
    );
        ...
        <div className="col-md-4 col-sm-6 pe-2 ps-2">
            ...
            <div className="theme-dark-bg d-block border-0 shadow-xss rounded-3 overflow-hidden mb-3">
                <div className="card-body d-block w-100 p-0 text-center">
                    ...
                    <div className="p-4">
                        ...
                        <h5 className="fw-700 font-xs start-date text-center">
                            About
                        </h5>
                        ...
                        <div className="d-flex justify-content-evenly mt-2">
                            {
                                apply == "Submit" ? <BadgeModal title={props.data} status={status} /> : <button onClick={handleOpenModal} style={{border:'none',}} className="p-2  bg-primary-gradiant  me-2 text-white text-center font-xssss fw-600 ls-1 rounded border-none">{apply}</button>
                            }
                        </div>
                    </div>
                </div>
            ...
            <Modal show={show} onHide={handleClose} style={{ overflowY: 'scroll' }}>
                <Modal.Header className='theme-dark-bg' closeButton>
                    <Modal.Title className='h4' style={{ fontWeight: "600" }}>Title : {props.data.contestData.title}</Modal.Title>
                </Modal.Header>
